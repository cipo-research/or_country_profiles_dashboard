...
#### Load and clean data ####
trade_by_country <- read_csv("fct_macroecon_flows_all_yearly.csv") |> 
  select(-`...1`, -m49_code, -sccai_code) |> distinct()
...
imports_to <- trade_by_country |> filter(flow_type== "imports_to_region")
exports_from <- trade_by_country |> filter(flow_type== "exports_from_region")
...
imports_to_region <- imports_to |> arrange(year, dollar_value) |>
  group_by(year) |>
  mutate(rank = rank(-dollar_value, ties.method = "min")) |>
  mutate(position = rank(-dollar_value, ties.method = "random"))

exports_from_region <- exports_from |> arrange(year, dollar_value) |>
  group_by(year) |>
  mutate(rank = rank(-dollar_value, ties.method = "min")) |>
  mutate(position = rank(-dollar_value, ties.method = "random"))

first_year_import <- imports_to_region$year |> min()
last_year_import <- imports_to_region$year |> max()

first_year_export <- exports_from_region$year |> min()
last_year_export <- exports_from_region$year |> max()
...
import_plot <- imports_plotting |>
  ...
  scale_x_continuous(limits=c(first_year - 3.5, last_year + 3.5), 
                     breaks = seq(from = first_year, to = last_year, by = 3)) +
  theme_minimal()+
  ...
  labs(title="Ranking of destinations for exports from Canada")+
  xlab("Year") +
  ylab("")
...
ggsave("imports_to_region_bump_plot.png", import_plot, bg = 'white', height = 5.5, width = 7.5, units = "in")





#### Exports to region (ie imports from Canada) ####

# filter on chosen years
exports_from_region <- exports_from_region |> 
  filter(year >= first_year & year <= last_year)

# bump plots require 2 or more data points for a country
# this filters out those with 1 or less
exports_plotting <- exports_from_region |>
  group_by(region) |>
  mutate(years=n())|>
  filter(years>1)
...
export_plot <- exports_plotting |>
  ...
  theme(text=element_text(family="sans"), 
        ...
        panel.background = element_blank(),
        panel.grid = element_blank()) +
  labs(title="Ranking of destinations for imports to Canada")+
  xlab("Year") +
  ylab("")
...
# save image to port into decks etc., done this way to ensure standard ratios
ggsave("exports_from_region_bump_plot.png", export_plot, bg = 'white', height = 5.5, width = 7.5, units = "in")
...
cdi <- trade_by_country |> filter(flow_type== "Canadian direct investment abroad - total book value")
fdi <- trade_by_country |> filter(flow_type== "Foreign direct investment in Canada - total book value")
...
cdi_plot <- cdi_plotting |>
  ...
  theme(text=element_text(family="sans"), 
        ...
        panel.background = element_blank(),
        panel.grid = element_blank()) +
  labs(title="Ranking of Canadian Direct Investment Abroad, by Total Book Value")+
  xlab("Year") +
  ylab("")
...
fdi_plot <- fdi_plotting |>
  ...
  theme(text=element_text(family="sans"), 
        ...
        panel.background = element_blank(),
        panel.grid = element_blank()) +
  labs(title="Ranking of Forgein Direct Investment in Canada, by Total Book Value")+
  xlab("Year") +
  ylab("")
